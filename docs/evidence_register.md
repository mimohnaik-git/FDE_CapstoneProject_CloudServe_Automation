...
| Deterministic routing | Build Specification; `AGENTS.md` §§14, 20 A5 | `src/route.py`; thresholds in `src/config.py` | `tests/test_route.py`, `tests/test_calibration.py`, pipeline tests | Stage 11 retained 0.80/0.30; Stage 16 routing accuracy 40%, automation 0%, escalation 100% | A5, A11 | **SAFE BUT NOT BUSINESS-VIABLE.** No validation auto-responses; escalation target ≤30% failed. |
...
| FastAPI interface | Operationalization requirement; `AGENTS.md` §26 Stage 17 | `src/api.py` | `tests/test_api.py` | Local smoke and regression evidence | A1, A2, A8, A11, A12 | **IMPLEMENTED / LOCALLY TESTED.** No authentication, authorization, rate-limit, or deployed availability evidence. |
